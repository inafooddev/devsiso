...
class TargetSeVtkpImport implements ToModel, WithHeadingRow, SkipsEmptyRows
{
    protected $errors = [];
    ...
    public function model(array $row)
    {
        ...
        // Validasi format
        if (empty($bulanRaw) || empty($distributorCode) || empty($salesmanCode) || empty($produkGrup)) {
            $this->errors[] = "Baris dilewati karena ada kolom wajib (Bulan, Distributor Code, Salesman Code, Produk Grup) yang kosong.";
            return null;
        }
        ...
        // Upsert data
        TargetSeVtkp::updateOrCreate(
            [
                'bulan' => $bulan,
                'distributor_code' => $distributorCode,
                'salesman_code' => $salesmanCode,
                'produk_grup' => $produkGrup,
            ],
            [
                'target' => $target,
            ]
