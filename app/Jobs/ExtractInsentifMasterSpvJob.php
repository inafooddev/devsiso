...
class ExtractInsentifMasterSpvJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        if ($this->batchId) {
            ...
            if ($batch) {
                $batch->addLog('info', '[1.5/X] Memulai agregasi Master SPV (MPP)...');
                $batch->addLog('warning', '[1.5/X] Membersihkan data lama Master SPV untuk bulan ' . $this->bulan);
            }
        }

        // 1. Bersihkan data bulan ini
        \Illuminate\Support\Facades\DB::table('insentif_master_spvs')->where('bulan', $this->bulan)->delete();

        if (isset($batch)) {
            $batch->addLog('info', '[1.5/X] Mengeksekusi kueri agregasi Master SPV...');
        }
        ...
        $query = "
            INSERT INTO insentif_master_spvs (
                bulan,
                region_name,
                area_name,
                cabang,
            ...
            WITH CabangGabungan AS (
                ...
                SELECT cabang 
                FROM insentif_master_distributors 
                WHERE bulan = ?
            )
            SELECT 
                ? as bulan,
        ...
        if (isset($batch)) {
            $batch->addLog('success', '[1.5/X] Sukses membuat Master SPV!');
        }
    }
}
