...
class SyncRptVisitAnRJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($this->batchId) {
            ...
            if ($batch) {
                ...
                $this->logMessage($batch, 'info', "Memulai proses Sync RPT Visit An R...");
            }
        } else {
            $this->logMessage(null, 'info', "Memulai proses Sync RPT Visit An R...");
        }

        try {
            // STEP 1: Fetch API
            $this->logMessage($batch, 'info', "Tahap 1: Mengunduh data dari API RPT_ANR...");
            
            $url = 'https://jobs.asiatop.co.id:9080/trx/export?block=RPT_ANR';
            ...
            foreach ($chunks as $chunk) {
                ...
                DB::table('rpt_visit_an_r')->insert($insertData);
            ...
            $this->logMessage($batch, 'success', "Proses Selesai. Sync RPT Visit An R sukses dijalankan.");
            if ($batch) $batch->update(['status' => 'completed']);
            Log::info("SyncRptVisitAnRJob selesai.");

        } catch (\Exception $e) {
            $this->logMessage($batch, 'error', "Terjadi kesalahan: " . $e->getMessage());
            if ($batch) $batch->update(['status' => 'failed']);
            Log::error("SyncRptVisitAnRJob Error: " . $e->getMessage());
            throw $e;
        }
    }
