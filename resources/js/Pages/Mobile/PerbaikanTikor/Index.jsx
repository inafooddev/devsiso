...
export default function Index({ outlets = [], sessionSalesCode, sessionSalesName }) {
    ...
    const openDetail = (outlet) => {
        ...
        const actLat = outlet.audit_latitude || outlet.latitude;
        const actLng = outlet.audit_longitude || outlet.longitude;
    ...
    return (
        <div className="w-full min-h-screen bg-slate-50 text-slate-800 flex flex-col relative pb-20">
                    ...
                    {filteredOutlets.length > 0 ? filteredOutlets.slice(0, displayLimit).map((outlet) => (
                                ...
                                <div className="flex-1 min-w-0">
                                    {outlet.status_perbaikan && (
                                        <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                                            <span className="text-[8px] px-1.5 py-0.5 rounded-md font-bold uppercase tracking-wider border bg-indigo-50 text-indigo-600 border-indigo-100/80">
                                                Telah Diperbaiki
                                            </span>
                                        </div>
                                    )}
                        ...
                        <div className="bg-white border border-slate-100 rounded-3xl py-12 px-6 text-center shadow-sm flex-1 flex flex-col items-center justify-center col-span-full">
                            ...
                            <p className="text-[10px] text-slate-400 mt-2 font-medium">
                                Belum ada daftar toko untuk sales ini.
                            </p>
                        </div>
                    )}
                </div>
