...
import {
    ...
    DocumentCheckIcon, ArrowLeftIcon
} from '@heroicons/react/24/outline';
import { ShieldExclamationIcon } from '@heroicons/react/24/solid';

import StoreCard from './Components/StoreCard';

export default function Index({ listPlan = [], sessionSupervisorCode, sessionSupervisorName }) {
    ...
    const handleLogin = (e) => {
        ...
        router.post('/mobile/call-plan/login', { supervisor_code: loginSupervisorCode }, {
            onError: (errors) => {
    ...
    const confirmLogout = () => {
        ...
        router.post('/mobile/call-plan/logout', {}, {
    ...
    const displayedData = useMemo(() => {
        ...
        let result = listPlan.filter(item => item.tanggal === selectedYMD);
    ...
    if (!sessionSupervisorCode) {
        return (
            <div className="w-full min-h-screen bg-gradient-to-br from-indigo-50 via-slate-50 to-indigo-100/50 flex items-center justify-center p-6">
                <Head title="Login Supervisor - Call Plan" />
                {toast && (
                ...
                <div className="w-full max-w-sm bg-white/90 backdrop-blur-lg border border-slate-200/50 rounded-3xl shadow-xl p-6 flex flex-col items-center animate-fade-in relative">
                    ...
                    <h2 className="text-sm md:text-base font-black uppercase tracking-wider text-slate-900 leading-tight text-center mb-6">Call Plan</h2>
    ...
    return (
        <div className="w-full min-h-screen bg-slate-50 text-slate-800 flex flex-col relative pb-6">
            <Head title="Call Plan" />
            {toast && (
                <div
            ...
            <div className="sticky top-0 z-30 bg-white/95 backdrop-blur-md border-b border-slate-100 shadow-sm shrink-0">
                <header className="px-4 py-3 flex items-center justify-between" style={{ paddingTop: 'calc(0.75rem + env(safe-area-inset-top, 0px))' }}>
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <h1 className="text-xs md:text-sm font-black uppercase tracking-wider text-slate-900 leading-tight">Call Plan</h1>
                        </div>
                    </div>
            ...
            <main className="flex-1 flex flex-col pt-4">
                <div className="grid grid-cols-1 gap-4 px-4">
                    {displayedData.length > 0 ? displayedData.slice(0, displayLimit).map((item, index) => (
                        <StoreCard key={item.customer_code} item={item} index={index} />
                    )) : (
