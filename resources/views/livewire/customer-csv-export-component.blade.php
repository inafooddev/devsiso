<div>
    <x-slot name="title">Data PDAMASTER (PRC Eska)</x-slot>
        ...
        <div class="bg-white shadow-xl rounded-2xl overflow-hidden">
            ...
            <div
                ...
                <div class="flex items-center space-x-2 flex-wrap">
                    ...
                    {{-- Tombol Export (Hanya muncul jika sudah difilter) --}}
                    @if ($isFiltered)
                ...
                {{-- Search Input (Hanya muncul jika sudah difilter) --}}
                @if ($isFiltered)
                    <div class="w-full sm:w-auto">
                        <input wire:model.live.debounce.500ms="search" type="text"
                            placeholder="Cari Nama Customer / No..."
                            ...
                            @forelse($customers as $row)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                        ...
                                        <div class="text-xs text-gray-500">Cabang: {{ $row->kodecabang }}</div>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm font-mono text-blue-600">
                                        {{ $row->custno }}
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                        <div class="font-medium">{{ $row->custname }}</div>
                                        <div class="text-xs text-gray-500 truncate max-w-xs"
                                            title="{{ $row->custadd1 }}">{{ $row->custadd1 }}</div>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                                        {{ $row->ccity }}
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-500">
                                        <div>Term: {{ $row->cterm }}</div>
                                        <div>Type: {{ $row->typeout }}</div>
                                        <div>Grp: {{ $row->grupout }}</div>
                                        <div>Hrg: {{ $row->gharga }}</div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
            ...
            @if ($isFiltered && method_exists($customers, 'links'))
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 class="text-lg leading-6 font-medium text-gray-900" id="modal-title">Filter Data
                                    PDAMASTER</h3>
                                <div class="mt-4 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select wire:model.live="selectedRegions" multiple
                                            class="block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2 border h-24">
                                            @foreach ($regionsOption as $r)
                                                <option value="{{ $r->region_code }}">{{ $r->region_name }}</option>
                                            @endforeach
                                        </select>
                                        <p class="text-[10px] text-gray-500 mt-1">*Tahan CTRL/CMD untuk pilih banyak
                                        </p>
                                    </div>
