<div>
    <x-slot name="title">Data Product Lines</x-slot>
    ...
    <div class="mx-auto px-4 sm:px-6 py-8 text-base-content">
        ...
        <x-card flush title="Product Lines" icon="cube" subtitle="Kelola kategori lini produk untuk pengelompokan item penjualan" class="pb-6">
            <x-slot:actions>
                <div class="flex flex-wrap items-center gap-3">
                    <div class="relative group mr-2">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari Line..." 
            ...
            {{-- Tabel Data --}}
            <x-ui.table loading="{{ false }}" empty="Tidak ada data product line ditemukan.">
                <x-slot:head>
                    <tr>
                        <th class="w-16">No</th>
                        <th>Kode Line</th>
                        <th>Nama Product Line</th>
                        <th>Dibuat Pada</th>
                ...
                @foreach ($lines as $index => $line)
                    <tr wire:key="line-{{ $line->line_id }}" class="group text-sm">
                        <td>
                            <span class="text-xs font-semibold text-base-content/40">{{ $lines->firstItem() + $index }}</span>
                        </td>
                        <td>
                            <span class="badge badge-sm badge-outline border-base-300 text-primary font-mono px-2 py-3 rounded-lg">{{ $line->line_id }}</span>
                        </td>
                        <td>
                            <span class="font-bold text-base-content/80 group-hover:text-primary transition-colors">{{ $line->line_name }}</span>
                        </td>
                        <td>
                            <div class="flex items-center gap-2 text-base-content/50">
                                <x-heroicon-s-calendar class="w-3.5 h-3.5" />
                                <span>{{ $line->created_at->format('d M Y') }}</span>
                            </div>
                        </td>
                        <td>
                            <div class="flex items-center gap-2 text-base-content/50">
                                <x-heroicon-s-clock class="w-3.5 h-3.5" />
                                <span>{{ $line->updated_at->diffForHumans() }}</span>
                            </div>
                        </td>
                        <td>
                            @canEdit('product-lines.index')
                            <div class="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                <button wire:click="openEditModal('{{ $line->line_id }}')" 
                                ...
                                <button wire:click="confirmDelete('{{ $line->line_id }}')" 
