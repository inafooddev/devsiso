<div class="flex-1 min-h-0 min-w-0 flex flex-col gap-3 md:gap-4 w-full h-full">
    <x-slot name="title">Data Master Product Brand</x-slot>
    ...
    <div class="bg-base-100 rounded-xl shadow-xl border border-base-300 flex-1 min-h-0 min-w-0 flex flex-col overflow-hidden">
        ...
        <div class="p-3 md:p-4 lg:p-5 border-b border-base-300 shrink-0 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-base-200/30">
            <div class="shrink-0 w-full sm:w-auto">
                <h2 class="text-base md:text-lg font-bold">Product Brand</h2>
                <p class="text-[10px] md:text-xs text-base-content/60 font-semibold uppercase tracking-wider mt-0.5">Kelola merk produk untuk pemasaran</p>
            </div>
            
            <div class="flex flex-wrap items-center justify-start sm:justify-end gap-2 md:gap-3 w-full sm:w-auto">
                {{-- Search --}}
                <x-ui.search-input wire:model.live.debounce.300ms="search" placeholder="Cari Brand..." />

                {{-- Action Buttons --}}
                <div class="flex flex-wrap items-center gap-1 md:gap-2">
                    @canEdit('product-brands.index')
                    ...
                    @canExport('product-brands.index')
            ...
            <table class="table table-sm table-zebra table-pin-rows w-full whitespace-nowrap">
                ...
                <tbody class="text-sm">
                    @forelse ($brands as $index => $brand)
                        <tr wire:key="brand-{{ $brand->brand_id }}" class="hover:bg-base-200/50 transition-colors group">
                            <th>{{ $brands->firstItem() + $index }}</th>
                            
                            {{-- Product Brand --}}
                            <td>
                                <div class="flex flex-col gap-0.5">
                                    <span class="font-bold text-[11px] text-base-content/90">{{ $brand->brand_name }}</span>
                                    <span class="text-[10px] text-base-content/50 font-mono uppercase">{{ $brand->brand_id }}</span>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="flex items-center gap-2 text-base-content/50">
                                    ...
                                    <span class="text-xs">{{ $brand->created_at->format('d M Y') }}</span>
                                </div>
                            </td>
                            ...
                            <td>
                                <div class="flex items-center gap-2 text-base-content/50">
                                    ...
                                    <span class="text-xs">{{ $brand->updated_at->diffForHumans() }}</span>
                                </div>
                            </td>
                            ...
                            <td class="text-center bg-base-200/40 border-l border-base-300 shadow-[inset_1px_0_0_rgba(0,0,0,0.02)]">
                                ...
                                <div class="flex items-center justify-center gap-1 transition-opacity duration-200">
                                    <x-ui.action-button type="edit" wire:click="openEditModal('{{ $brand->brand_id }}')" class="btn-square" title="Edit" />
                                    <x-ui.action-button type="delete" wire:click="confirmDelete('{{ $brand->brand_id }}')" class="btn-square" title="Hapus" />
                                </div>
                                @else
        ...
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-base-300 bg-base-200/30 shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-bold text-lg leading-none">{{ $isEditing ? 'Edit Product Brand' : 'Tambah Brand Baru' }}</h3>
                        <p class="text-[11px] text-base-content/50 mt-1 uppercase tracking-wider font-semibold">{{ $isEditing ? 'Perbarui informasi merk produk' : 'Daftarkan merk produk baru' }}</p>
                    </div>
                </div>
            ...
            <form wire:submit.prevent="save">
                <div class="p-6 space-y-6 bg-base-100">
                    <div class="space-y-1.5">
                        <label for="brand_id" class="text-xs font-bold uppercase tracking-wider text-base-content/50 ml-1">ID Product Brand</label>
                        <div class="relative group">
                            <input wire:model.blur="brand_id" type="text" id="brand_id" placeholder="Cth: BRD-001"
                                   class="input input-bordered w-full bg-base-200 border-base-300 rounded-2xl focus:ring-2 focus:ring-primary/50 transition-all duration-300 @error('brand_id') input-error @enderror"
                                   {{ $isEditing ? 'disabled' : '' }}>
                            @if($isEditing)
                    ...
                    <div class="space-y-1.5">
                        <label for="brand_name" class="text-xs font-bold uppercase tracking-wider text-base-content/50 ml-1">Nama Product Brand</label>
                        <input wire:model.blur="brand_name" type="text" id="brand_name" placeholder="Cth: Merk Unggulan"
                               class="input input-bordered w-full bg-base-200 border-base-300 rounded-2xl focus:ring-2 focus:ring-primary/50 transition-all duration-300 @error('brand_name') input-error @enderror">
                        @error('brand_name') <span class="text-error text-[10px] font-medium ml-1 flex items-center gap-1 mt-1"><x-heroicon-s-exclamation-circle class="w-3 h-3" /> {{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3 px-6 py-5 border-t border-base-300 bg-base-200/30 shrink-0">
                    ...
                    <button type="submit" class="btn btn-primary rounded-xl px-10 normal-case shadow-sm shadow-primary/20 gap-2">
                        <span wire:loading.remove wire:target="save">{{ $isEditing ? 'Simpan Perubahan' : 'Tambah Brand' }}</span>
    ...
    <div x-data="{ open: @entangle('isDeleteModalOpen') }" 
        ...
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
            ...
            <div class="p-8 text-center text-base-content">
                ...
                <h3 class="text-xl font-bold mb-2 leading-none text-base-content">Hapus Product Brand?</h3>
                <p class="text-[13px] text-base-content/50 leading-relaxed px-4">Seluruh data yang terkait dengan merk ini akan terdampak. Tindakan ini <span class="text-error font-bold italic">permanen</span>.</p>
            </div>
        ...
        <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
            ...
            <div class="flex items-center justify-between px-6 py-5 border-b border-base-300 bg-base-200/30 shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-bold text-lg leading-none">Import Product Brand</h3>
