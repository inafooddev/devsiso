<div>
    ...
    <div class="mx-auto px-4 sm:px-6 py-8">
        ...
        <x-card flush title="Master Supervisor" icon="user-group" subtitle="Kelola data pengawas area operasional" class="pb-6">
            <x-slot:actions>
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative group">
                        ...
                        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari Supervisor..." 
            ...
            {{-- Tabel Data --}}
            <x-ui.table loading="{{ false }}" empty="Tidak ada data supervisor ditemukan.">
                <x-slot:head>
                    <tr>
                        <th class="w-16">No</th>
                        <th>Kode SPV</th>
                ...
                @foreach ($supervisors as $index => $supervisor)
                    <tr wire:key="supervisor-{{ $supervisor->supervisor_code }}" class="group">
                        <td>
                            <span class="text-xs font-semibold text-base-content/40">{{ $supervisors->firstItem() + $index }}</span>
                        </td>
                        <td>
                            <div class="flex items-center gap-2">
                                <span class="badge badge-sm badge-outline border-base-300 text-primary font-bold px-2 py-3 rounded-lg">{{ $supervisor->supervisor_code }}</span>
                            </div>
                        </td>
                        <td>
                            <span class="font-medium text-base-content/80 group-hover:text-primary transition-colors">{{ $supervisor->supervisor_name }}</span>
                        </td>
                        <td>
                            <div class="flex flex-col gap-0.5">
                                <div class="flex items-center gap-1.5">
                                    <x-heroicon-s-map-pin class="w-3.5 h-3.5 text-base-content/30" />
                                    <span class="text-sm text-base-content/70 font-medium">{{ $supervisor->area->area_name ?? 'N/A' }}</span>
                                </div>
                                <span class="text-[10px] uppercase tracking-wider text-base-content/40 ml-5 font-bold">{{ $supervisor->area->region->region_name ?? 'N/A' }}</span>
                            </div>
                        </td>
                        ...
                        <td>
                            ...
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button wire:click="confirmDelete('{{ $supervisor->supervisor_code }}')" 
