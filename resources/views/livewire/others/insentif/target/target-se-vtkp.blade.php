        ...
        <div class="flex flex-wrap items-center justify-start sm:justify-start gap-2 md:gap-3 w-full sm:w-auto">
            ...
            <div class="relative group grow sm:grow-0">
                ...
                <input wire:model.live.debounce.300ms="search" type="text"
                       placeholder="Cari salesman/dist..."
        ...
        <table class="table table-xs w-full border-collapse">
            <thead class="bg-base-200">
                <tr class="h-10">
                    ...
                    <th class="border border-base-300 bg-base-200 text-center align-middle sticky top-0 z-10">Distributor</th>
                    <th class="border border-base-300 bg-base-200 text-center align-middle sticky top-0 z-10">Salesman</th>
            ...
            <tbody>
                @forelse ($data as $index => $item)
                    <tr wire:key="row-{{ $index }}" class="hover:bg-base-200 transition-colors">
                        ...
                        <td class="border border-base-300 font-mono text-center">{{ $item->distributor_code }}</td>
                        <td class="border border-base-300 font-mono text-center">{{ $item->salesman_code }}</td>
                        ...
                        <td class="border border-base-300 text-center">
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button wire:click="openSwapModal('{{ addslashes($item->distributor_code) }}', '{{ addslashes($item->salesman_code) }}')" 
                                        class="btn btn-xs btn-square btn-outline btn-warning" 
                                        title="Tukar Target (Swap)">
                                    <x-heroicon-s-arrows-right-left class="w-4 h-4" />
                                </button>

                                <button wire:click="openEditModal('{{ addslashes($item->distributor_code) }}', '{{ addslashes($item->salesman_code) }}')" 
                                ...
                                <button wire:click="deleteData('{{ addslashes($item->distributor_code) }}', '{{ addslashes($item->salesman_code) }}')" 
                                        onclick="return confirm('Yakin ingin menghapus data Salesman {{ addslashes($item->salesman_code) }} di kuartal ini?')"
                ...
                @empty
                    <tr>
                        <td colspan="{{ count($productGroups) + 4 }}" class="text-center py-8 text-base-content/50">Tidak ada data ditemukan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
            ...
            <div x-show="isImportOpen"
                ...
                <div class="flex items-center justify-between px-6 py-5 border-b border-base-300 bg-base-200/30 rounded-t-3xl">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-bold text-lg leading-none">Import Target SE VTKP</h3>
            ...
            <div x-show="isEditOpen"
                ...
                <div class="flex items-center justify-between px-6 py-5 border-b border-base-300 bg-base-200/30 rounded-t-3xl">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-bold text-lg leading-none">Edit Target SE VTKP</h3>
                    ...
                    <div class="p-6 space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-1.5">
                                ...
                                <input type="text" class="input input-sm input-bordered w-full bg-base-200/50 font-mono font-semibold" wire:model="editDistributorCode" readonly>
                            </div>
                            <div class="space-y-1.5">
                                <label class="text-xs font-bold uppercase tracking-wider text-base-content/50 ml-1">Salesman</label>
                                <input type="text" class="input input-sm input-bordered w-full bg-base-200/50 font-mono font-semibold" wire:model="editSalesmanCode" readonly>
                            </div>
                        </div>
            ...
            <div x-show="$wire.isSwapModalOpen"
                ...
                <div class="flex items-center justify-between px-6 py-5 border-b border-base-300 bg-warning/10 rounded-t-3xl">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-base-content/50 mt-1 uppercase tracking-wider font-semibold">Tukar Nilai Target antar SE (Kuartal Ini)</p>
                        </div>
                    </div>
                ...
                <form wire:submit.prevent="processSwap">
                    <div class="p-6 space-y-5">
                        ...
                        @if($swapSourceSalesmanCode)
                        <div class="bg-base-200 p-4 rounded-xl space-y-2 border border-base-300">
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-base-content/70">Tahun & Kuartal</span>
                                <span class="font-bold">{{ $yearFilter }} - {{ $quarterFilter }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-base-content/70">Distributor</span>
                                <span class="font-bold">{{ $swapSourceDistributorCode }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-base-content/70">SE Sumber (A)</span>
                                <span class="font-bold text-primary">{{ $swapSourceSalesmanCode }}</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select wire:model="swapTargetSalesmanCode" class="select select-bordered w-full rounded-xl focus:outline-none focus:ring-2 focus:ring-warning/50">
                                <option value="">-- Pilih Salesman (SE) --</option>
                                @foreach($swapListSE as $se)
                                    <option value="{{ $se }}">{{ $se }}</option>
                                @endforeach
                            </select>
                            @error('swapTargetSalesmanCode') <span class="text-error text-[10px] font-medium ml-1">{{ $message }}</span> @enderror
                        </div>
                        
                        <p class="text-xs text-base-content/60 text-center mt-2">
                            <strong>SELURUH</strong> nilai target per Produk Grup milik <strong>SE A</strong> akan diberikan ke <strong>SE B</strong>, dan sebaliknya, pada kuartal ini.
                        </p>
                        @endif
