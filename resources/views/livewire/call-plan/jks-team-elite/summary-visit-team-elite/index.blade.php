<div>
    ...
    <div class="grid grid-cols-7 gap-4 mb-6">
        ...
        <div class="rounded-2xl p-4 shadow-sm border border-base-200 flex flex-col relative overflow-hidden bg-base-100">
            ...
            <div class="text-xs text-base-content/60 mt-3 z-10 flex flex-col gap-1 font-medium">
                <span class="flex justify-between border-b border-base-200 pb-1">
                    ...
                    <span class="text-base-content font-bold">{{ number_format($kpiData['total_toko'] ?? 0, 0, ',', '.') }}</span>
                </span>
        ...
        <div class="rounded-2xl p-4 shadow-sm border border-base-200 flex flex-col relative overflow-hidden bg-base-100">
            ...
            <div class="text-sm text-base-content/70 font-medium z-10">Toko Order</div>
            <div class="text-3xl font-bold mt-1 z-10 text-success">{{ number_format($kpiData['total_toko_order'] ?? 0, 0, ',', '.') }}</div>
            <div class="text-xs text-base-content/60 mt-3 z-10 flex flex-col gap-1 font-medium">
                <span class="flex justify-between border-b border-base-200 pb-1">
                    <span>Total Toko:</span>
                    <span class="text-base-content font-bold">{{ number_format($kpiData['total_toko'] ?? 0, 0, ',', '.') }}</span>
                </span>
                <span class="flex justify-between pt-0.5 {{ (($kpiData['total_toko'] ?? 0) - ($kpiData['total_toko_order'] ?? 0)) > 0 ? 'text-error' : 'text-success' }}">
                    <span>Gap:</span>
                    <span class="font-bold">{{ number_format(($kpiData['total_toko'] ?? 0) - ($kpiData['total_toko_order'] ?? 0), 0, ',', '.') }}</span>
                </span>
            </div>
        </div>
        ...
        <div class="rounded-2xl p-4 shadow-sm border border-base-200 flex flex-col relative overflow-hidden bg-base-100">
            ...
            <div class="text-sm text-base-content/70 font-medium z-10">Total 1. RWO</div>
            <div class="text-3xl font-bold mt-1 z-10 text-info">{{ number_format($kpiData['total_rwo'] ?? 0, 0, ',', '.') }}</div>
            <div class="text-xs text-base-content/60 mt-3 z-10 flex flex-col gap-1 font-medium">
                <span class="flex justify-between border-b border-base-200 pb-1">
                    <span>Total Toko:</span>
                    <span class="text-base-content font-bold">{{ number_format($kpiData['total_toko'] ?? 0, 0, ',', '.') }}</span>
                </span>
                <span class="flex justify-between pt-0.5 {{ (($kpiData['total_toko'] ?? 0) - ($kpiData['total_rwo'] ?? 0)) > 0 ? 'text-error' : 'text-success' }}">
                    <span>Gap:</span>
                    <span class="font-bold">{{ number_format(($kpiData['total_toko'] ?? 0) - ($kpiData['total_rwo'] ?? 0), 0, ',', '.') }}</span>
                </span>
            </div>
        </div>
        ...
        <div class="rounded-2xl p-4 shadow-sm border border-base-200 flex flex-col relative overflow-hidden bg-base-100">
            ...
            <div class="text-sm text-base-content/70 font-medium z-10">Total 2. PNR</div>
            <div class="text-3xl font-bold mt-1 z-10 text-warning">{{ number_format($kpiData['total_pnr'] ?? 0, 0, ',', '.') }}</div>
            <div class="text-xs text-base-content/60 mt-3 z-10 flex flex-col gap-1 font-medium">
                <span class="flex justify-between border-b border-base-200 pb-1">
                    <span>Total Toko:</span>
                    <span class="text-base-content font-bold">{{ number_format($kpiData['total_toko'] ?? 0, 0, ',', '.') }}</span>
                </span>
                <span class="flex justify-between pt-0.5 {{ (($kpiData['total_toko'] ?? 0) - ($kpiData['total_pnr'] ?? 0)) > 0 ? 'text-error' : 'text-success' }}">
                    <span>Gap:</span>
                    <span class="font-bold">{{ number_format(($kpiData['total_toko'] ?? 0) - ($kpiData['total_pnr'] ?? 0), 0, ',', '.') }}</span>
                </span>
            </div>
        </div>
        ...
        <div class="rounded-2xl p-4 shadow-sm border border-base-200 flex flex-col relative overflow-hidden bg-base-100">
            ...
            <div class="text-sm text-base-content/70 font-medium z-10">Total 3. NGVO</div>
            <div class="text-3xl font-bold mt-1 z-10 text-secondary">{{ number_format($kpiData['total_ngvo'] ?? 0, 0, ',', '.') }}</div>
            <div class="text-xs text-base-content/60 mt-3 z-10 flex flex-col gap-1 font-medium">
                <span class="flex justify-between border-b border-base-200 pb-1">
                    <span>Total Toko:</span>
                    <span class="text-base-content font-bold">{{ number_format($kpiData['total_toko'] ?? 0, 0, ',', '.') }}</span>
                </span>
                <span class="flex justify-between pt-0.5 {{ (($kpiData['total_toko'] ?? 0) - ($kpiData['total_ngvo'] ?? 0)) > 0 ? 'text-error' : 'text-success' }}">
                    <span>Gap:</span>
                    <span class="font-bold">{{ number_format(($kpiData['total_toko'] ?? 0) - ($kpiData['total_ngvo'] ?? 0), 0, ',', '.') }}</span>
                </span>
            </div>
        </div>
        ...
        <div class="rounded-2xl p-4 shadow-sm border border-base-200 flex flex-col relative overflow-hidden bg-base-100">
            ...
            <div class="text-sm text-base-content/70 font-medium z-10">Total NOO</div>
            <div class="text-3xl font-bold mt-1 z-10 text-accent">{{ number_format($kpiData['total_noo'] ?? 0, 0, ',', '.') }}</div>
            <div class="text-xs text-base-content/60 mt-3 z-10 flex flex-col gap-1 font-medium">
    ...
    <x-card title="Summary Visit Team Elite" icon="document-text" class="mb-4" flush="true">
        ...
        <x-ui.table striped hover sticky="true" empty="Tidak ada data." class="max-h-[60vh] overflow-y-auto border-x-0 border-b-0 rounded-none shadow-none mt-2">
            ...
            @foreach($dataKunjungan as $row)
                <tr class="{{ str_contains(strtoupper($row->keterangan ?? ''), 'NOO') ? '!bg-warning/20 hover:!bg-warning/30' : '' }}">
                    ...
                    <td class="max-w-[150px] truncate" title="{{ $row->team_code }} - {{ $row->team_name }}">{{ $row->team_code }} - {{ $row->team_name }}</td>
                    ...
                    <td class="max-w-[200px] truncate" title="{{ $row->custname }}">{{ $row->custname }}</td>
                    <td class="max-w-[200px] text-xs truncate" title="{{ $row->address }}">{{ $row->address }}</td>
        ...
        <div class="p-8 text-center text-base-content/60">
            ...
            <p>Silakan sesuaikan filter di atas lalu klik tombol <strong>Filter</strong> untuk menampilkan data.</p>
        </div>
        @endif
    </x-card>
</div>
