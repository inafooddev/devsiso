                ...
                @if ($isFiltered)
                    <div class="w-full sm:w-auto">
                        <input wire:model.live.debounce.500ms="search" type="text"
                            ...
                            class="w-full sm:w-64 border-gray-300 rounded-md shadow-sm m-4 p-2 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-sm">
                    </div>
                @endif
            </div>
            ...
            <div class="overflow-x-auto">
                @if (!$isFiltered)
                    ...
                    <div class="p-8 text-center text-gray-500">
                        ...
                        <p class="mt-1 text-sm text-gray-500">Klik tombol "Filter" untuk memilih Region, Area, dan
                            Distributor.</p>
                        <button @click="showFilterModal = true"
                            ...
                            @forelse($customers as $row)
                                <tr class="hover:bg-gray-50 transition-colors duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <div class="text-gray-900 text-xs">{{ $row->region_name }}</div>
                                        <div class="text-xs text-gray-500">{{ $row->area_name }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {{ $row->kodecabang }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $row->distributor_name }}</td>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate"
                                        title="{{ $row->custadd1 }}">{{ $row->custadd1 ?? '-' }}</td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-10 text-center text-gray-500">
                                        <h3 class="text-sm font-medium text-gray-900">Tidak Ada Data</h3>
                                        <p class="mt-1 text-sm text-gray-500">Tidak ada data customer yang cocok dengan
                                            kriteria filter.</p>
                                    </td>
                                </tr>
                            @endforelse
                                ...
                                <div class="mt-4 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select wire:model.live="selectedArea"
                                            class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md border"
                                            @if (empty($selectedRegion)) disabled @endif>
                                    ...
                                    <div>
                                        ...
                                        <select wire:model.live="selectedDistributor"
                                            class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md border"
                                            @if (empty($selectedArea)) disabled @endif>
