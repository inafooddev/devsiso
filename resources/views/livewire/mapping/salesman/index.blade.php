<div>
    <x-slot name="title">Pemetaan Salesman</x-slot>
        ...
        <div class="bg-white shadow-xl rounded-2xl overflow-hidden">
            ...
            <div class="px-6 py-4 border-b bg-gray-50 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
                <div class="flex items-center space-x-2 flex-wrap">
                    ...
                    <a href="{{ asset('templates/salesman_mapping_template.xlsx') }}" download class="inline-flex items-center px-4 py-2 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-600 active:bg-yellow-700 focus:outline-none focus:border-yellow-700 focus:ring ring-yellow-300 disabled:opacity-50 transition ease-in-out duration-150 shadow-sm">
                    ...
                    <!--<a href="{{ route('salesman-mappings.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150 shadow-md hover:shadow-lg">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                        Tambah Manual
                    </a>-->
                </div>
                <div class="w-full sm:w-auto">
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari Kode/Nama Salesman..." class="w-full sm:w-64 form-input border-gray-300 rounded-md shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                </div>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-blue-600 uppercase tracking-wider">Kode SE Dist</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Salesman (Dist)</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-green-600 uppercase tracking-wider">Kode SE Pcr</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Salesman (Prc)</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($mappings as $index => $mapping)
                                <tr wire:key="mapping-{{ $mapping->id }}" class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $mapping->masterDistributor->distributor_code ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $mapping->masterDistributor->distributor_name ?? 'N/A' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-blue-600">{{ $mapping->salesman_code_dist }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $mapping->salesman_name_dist }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-green-600">{{ $mapping->salesman_code_prc ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $mapping->principalSalesman->salesman_name ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $mapping->created_at->format('d M Y, H:i') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                                        <button wire:click="edit('{{ $mapping->id }}')" class="inline-flex items-center justify-center h-8 w-8 rounded-full text-indigo-600 hover:text-indigo-900 hover:bg-indigo-100 transition-colors duration-200" title="Edit">
            ...
            <div x-show="open" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form wire:submit.prevent="applyFilters">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Filter Pemetaan Salesman</h3>
                        <div class="space-y-4">
                            <div>
   ...
   <!-- Modal Edit (Fokus Pemetaan Principal) -->
    <div x-data="{ open: @entangle('isEditModalOpen').live }" x-show="open" x-cloak class="fixed z-20 inset-0 overflow-y-auto">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"></div>          
            <div x-show="open" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form wire:submit.prevent="update">
                    <div class="bg-white px-6 py-6">
                        <h3 class="text-lg font-bold text-gray-900 border-b mb-4 mt-4 pb-4 pt-4">Edit Mapping Salesman</h3>
                        <div class="space-y-4">
                            <!-- Informasi Read-Only -->
                            <div class="bg-gray-50 p-3 rounded-md border border-gray-200">
                                <p class="text-xs text-gray-500 uppercase font-bold mb-1">Distributor</p>
                                <p class="text-sm text-gray-800 font-medium">{{ $distributor_name }}</p>
                            </div>

                            <div class="grid grid-cols-2 gap-4">
                                <div class="bg-gray-50 p-3 rounded-md border border-gray-200">
                                    <p class="text-xs text-gray-500 uppercase font-bold mb-1">Kode Salesman Dist</p>
                                    <p class="text-sm text-blue-600 font-mono font-bold">{{ $salesman_code_dist }}</p>
                                </div>
                                <div class="bg-gray-50 p-3 rounded-md border border-gray-200">
                                    <p class="text-xs text-gray-500 uppercase font-bold mb-1">Nama Salesman Dist</p>
                                    <p class="text-sm text-gray-800 font-medium">{{ $salesman_name_dist }}</p>
                                </div>
                            </div>

                            <!-- Input yang bisa diedit (Pemetaan) -->
                            <div class="pt-2 bg-gray-50 p-4 rounded-md border border-gray-200">
                                <label class="block text-sm font-bold text-indigo-700 mb-1">Salesman Principal</label>
                                <select wire:model="salesman_code_prc" class=" pb-4 pt-4 w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                    <option value="">-- Tanpa Mapping --</option>
                                    @foreach($principalSalesmans as $ps)
                                        <option value="{{ $ps->salesman_code }}">{{ $ps->salesman_name }} ({{ $ps->salesman_code }})</option>
                                    @endforeach
                                </select>
                                @error('salesman_code_prc') <span class="text-xs text-red-600">{{ $message }}</span> @enderror
                            </div>
                        </div>
                    </div>

                    <div class="bg-gray-50 px-6 py-4 flex flex-row-reverse">
                        <button type="submit" wire:loading.attr="disabled" class="inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-white font-medium hover:bg-indigo-700 ml-3 text-sm">
                            <span wire:loading.remove wire:target="update">Update Pemetaan</span>
                            <span wire:loading wire:target="update">Memproses...</span>
                        </button>
                        <span class="ml-2"></span>
                        <button @click="open = false" type="button" class="inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-gray-700 font-medium hover:bg-gray-50 text-sm">Batal</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="open" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form wire:submit.prevent="import">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Impor Pemetaan Salesman</h3>
                        <div class="space-y-4">
                            <div>
                            ...
                            <p class="text-sm text-gray-500">
                                Pastikan format file sesuai. Kolom: 
                                [A] Distributor Code, 
                                [B] Salesman Code Dist., 
                                [C] Salesman Name Dist., 
                                [D] Salesman Code Prc.
                            </p>
                        </div>
                    </div>
