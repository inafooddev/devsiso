<div class="flex-1 flex flex-col w-full h-full min-h-0">
    <x-slot name="title">Jobs - Sync Customer Eska</x-slot>
        ...
        <div class="bg-base-100 rounded-xl shadow-xl border border-base-300 flex-1 min-h-0 min-w-0 flex flex-col overflow-hidden">
            ...
            <div class="p-3 md:p-4 lg:p-5 border-b border-base-300 shrink-0 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-base-200/30">
                <div class="shrink-0 w-full lg:w-auto">
                    <h2 class="text-base md:text-lg font-bold">Pemrosesan Data Sync Customer Eska</h2>
                    <p class="text-[10px] md:text-xs text-base-content/60 font-semibold uppercase tracking-wider mt-0.5">Sinkronisasi Data Customer dari API</p>
                </div>

                <div class="flex flex-wrap items-center justify-start lg:justify-end gap-2 md:gap-3 w-full lg:w-auto">
                    <!-- Dropdown Region -->
                    <select wire:model="selectedRegion" class="select select-sm select-bordered w-full max-w-xs" {{ in_array($batchStatus, ['pending', 'processing']) ? 'disabled' : '' }}>
                        <option value="all">Semua Region</option>
                        <option value="CSTINAJWA1">CSTINAJWA1</option>
                        <option value="CSTINAJWA2">CSTINAJWA2</option>
                        ...
                        @if(empty($logLines))
                            <div class="flex flex-col items-center justify-center h-full text-slate-500 space-y-3">
                                ...
                                <div class="text-center">
                                    ...
                                    <p class="text-xs mt-1">Silakan pilih Region dan klik "Mulai Proses" untuk memperbarui data Customer Eska.</p>
                                </div>
                            </div>
                        @else
