<div>
    ...
    <div class="mx-auto px-6 py-8" x-data="{ 
        ...
        <div class="bg-white shadow-xl rounded-2xl overflow-hidden mb-8">
            ...
            <div class="px-6 py-4 border-b bg-gray-50 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
                 <div class="flex items-center space-x-2">
                    ...
                    {{-- Tombol Export (Memicu Modal Export) --}}
                    @if($isFiltered)
                    ...
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left font-medium text-gray-600 uppercase whitespace-nowrap">Kode Produk</th>
                                <th class="px-4 py-3 text-left font-medium text-gray-600 uppercase whitespace-nowrap">Nama Produk</th>
                                <th class="px-4 py-3 text-center font-medium text-gray-600 uppercase whitespace-nowrap">UOM (1-3)</th>
                                <th class="px-4 py-3 text-center font-medium text-gray-600 uppercase whitespace-nowrap">Conv (2-3)</th>
                                <th class="px-4 py-3 text-right font-medium text-gray-600 uppercase whitespace-nowrap">Sell Price 1</th>
                                <th class="px-4 py-3 text-right font-medium text-gray-600 uppercase whitespace-nowrap">Sell Price 2</th>
                                <th class="px-4 py-3 text-right font-medium text-gray-600 uppercase whitespace-nowrap">Sell Price 3</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($products as $row)
                            @php
                                // Kalkulasi harga di view untuk display
                                $sp1 = ($row->price_zone1 > 0) ? ($row->price_zone1 / 1.11) : 0;
                                $sp2 = ($sp1 > 0 && $row->conv_unit2 > 0) ? ($sp1 / $row->conv_unit2) : 0;
                                $sp3 = ($sp1 > 0 && $row->conv_unit1 > 0) ? ($sp1 / $row->conv_unit1) : 0;
                            @endphp
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3 text-center text-xs text-gray-500">
                                    {{ $row->uom1 }} / {{ $row->uom2 }} / {{ $row->uom3 }}
                                </td>
                                <td class="px-4 py-3 text-center text-xs text-gray-500">
                                    {{ number_format($row->conv_unit3,0) }} / {{ number_format($row->conv_unit2,0) }}
                                </td>
                                <td class="px-4 py-3 text-right font-mono">{{ number_format($sp1, 2, ',', '.') }}</td>
                                <td class="px-4 py-3 text-right font-mono">{{ number_format($sp2, 2, ',', '.') }}</td>
                                <td class="px-4 py-3 text-right font-mono">{{ number_format($sp3, 2, ',', '.') }}</td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="8" class="px-6 py-10 text-center text-gray-500">Data tidak ditemukan.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        {{-- MODAL FILTER UTAMA (Region/Area/Distributor) --}}
             ...
             <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Filter Data Produk</h3>
                        <div class="space-y-4">
                            <div>
        ...
        {{-- MODAL EXPORT (Filter Produk Multi-Select) --}}
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="text-xs text-gray-500 mb-2">
                            Pilih produk tertentu atau biarkan kosong untuk semua produk.
                        </div>

                        <select wire:model="selectedProducts" multiple 
                            class="mt-1 block w-full border-gray-300 rounded-md shadow-sm 
                            focus:ring-green-500 focus:border-green-500 sm:text-sm p-2 border h-96">

                            @foreach($productOptions as $prod)
                                <option value="{{ $prod->product_code_dist }}">
                                    {{ $prod->product_code_dist }} - {{ $prod->product_name_dist }}
                                </option>
                            @endforeach
                        </select>

                        <p class="text-xs text-gray-500 mt-1">
                            Tahan tombol CTRL (Windows) atau CMD (Mac) untuk memilih banyak.
                        </p>
                    </div>
                </div>
            </div>
