        ...
        <div class="flex-none p-4 md:p-6 border-b border-base-200">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div class="flex flex-col gap-1">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                            <x-heroicon-s-document-magnifying-glass class="w-5 h-5 text-primary" />
                        </div>
                        <div>
                            <h2 class="text-lg md:text-xl font-bold text-base-content leading-none">Unmapped Products</h2>
                            <p class="text-xs text-base-content/60 mt-1">Laporan produk distributor yang belum memiliki pemetaan ke produk principal</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2 w-full md:w-auto">
                    {{-- Search --}}
                    <x-ui.search-input wire:model.live.debounce.300ms="search" placeholder="Cari kode/nama produk..." />
                    ...
                    <table class="table table-sm table-pin-rows table-pin-cols w-full">
                        <thead>
                            <tr class="bg-base-200/50">
                                ...
                                <th class="text-xs">Kode Produk (Dist)</th>
                                <th class="text-xs">Nama Produk (Dist)</th>
                                <th class="w-24 text-center text-xs">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse ($products as $index => $product)
                                <tr wire:key="product-{{ $product->distributor_code }}-{{ $product->product_code }}" class="hover:bg-base-200/50 transition-colors text-sm">
                                    <td class="text-center font-medium text-base-content/50">
                                        {{ $products->firstItem() + $index }}
                                    </td>
                                    <td>
                                        <div class="font-bold text-base-content/90">{{ $product->distributor_name }}</div>
                                        <div class="text-xs text-base-content/50 font-mono mt-0.5">{{ $product->distributor_code }}</div>
                                    </td>
                                    <td>
                                        <span class="font-mono text-base-content/70">{{ $product->product_code }}</span>
                                    </td>
                                    <td>
                                        <span class="font-medium text-base-content/80">{{ $product->product_name }}</span>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="5" class="text-center py-8 text-base-content/50">
                                        <div class="flex flex-col items-center justify-center gap-2">
                                            ...
                                            <span>Semua produk untuk periode ini sudah memiliki pemetaan.</span>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
