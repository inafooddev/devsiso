<div>
        ...
        <div class="bg-white shadow-xl rounded-2xl overflow-hidden">
            ...
            <div class="px-6 py-4 border-b bg-gray-50 flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
                <div class="flex items-center space-x-2 flex-wrap">
                    ...
                    <a href="{{ asset('templates/salesman_template.xlsx') }}" download class="inline-flex items-center px-4 py-2 bg-yellow-500 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-yellow-600 active:bg-yellow-700 focus:outline-none focus:border-yellow-700 focus:ring ring-yellow-300 disabled:opacity-50 transition ease-in-out duration-150 shadow-sm">
                    ...
                    <a href="{{ route('salesmans.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150 shadow-md hover:shadow-lg">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Salesman Code</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Salesman Name</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($salesmans as $index => $salesman)
                                <tr wire:key="salesman-{{ $salesman->salesman_code }}" class="hover:bg-gray-50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $salesman->created_at->format('d M Y, H:i') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                                        <button wire:click="edit('{{ $salesman->distributor_code }}', '{{ $salesman->salesman_code }}')" class="text-indigo-600 hover:text-indigo-900 transition-colors" title="Edit Data">
                                                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </button>
                                        <button
                                            wire:click="confirmDelete('{{ $salesman->salesman_code }}', '{{ $salesman->distributor_code }}')"
                                            class="inline-flex items-center justify-center h-8 w-8 rounded-full text-red-600 hover:text-red-900 hover:bg-red-100 transition-colors duration-200"
                                            title="Hapus"
                                        >
            ...
            <div x-show="open" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form wire:submit.prevent="applyFilters">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Filter Salesman</h3>
                        <div class="space-y-4">
                            <div>
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="open" class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form wire:submit.prevent="import">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4">Impor Data Salesman</h3>
                        <div class="space-y-4">
                            <div>
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-lg leading-6 font-medium text-gray-900">Hapus Salesman</h3>
    ...
    <!-- MODAL EDIT SALESMAN -->
    <div x-data="{ open: @entangle('isEditModalOpen') }" x-show="open" x-cloak class="fixed z-30 inset-0 overflow-y-auto">
                ...
                <form wire:submit.prevent="update">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="flex items-center justify-between border-b pb-3 mb-5">
                            <h3 class="text-lg font-bold text-gray-900">Ubah Salesman</h3>
                        ...
                        <div class="space-y-3 mt-4">
                            ...
                            <div>
                                <label class="block text-[11px] font-semibold text-gray-500 tracking-wide">
                                    Kode Distributor
                                </label>
                                <input
                                    type="text"
                                    wire:model="distributor_code"
                                    readonly
                                    class="mt-1.5 block w-full h-9 px-3 py-2
                                        bg-gray-50 border border-gray-300 rounded-md
                                        text-sm text-gray-600 font-medium
                                        cursor-not-allowed focus:ring-0 focus:border-gray-300">
                            </div>

                            <!-- Salesman Code -->
                            <div class='mt-2'>
                                <label class="block text-[11px] font-semibold text-gray-700 tracking-wide">
                                    Kode Salesman <span class="text-red-500">*</span>
                                </label>
                                <input
                                    type="text"
                                    wire:model="salesman_code"
                                    class="mt-1.5 block w-full h-9 px-3 py-2
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">
                            <span wire:loading wire:target="update" class="mr-2">...</span> Simpan Perubahan
                        </button>
                        <button @click="open = false" type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:w-auto sm:text-sm">Batal</button>
                    </div>
                </form>
